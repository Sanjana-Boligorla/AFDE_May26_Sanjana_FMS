...
import { Save, ArrowLeft, User, Mail, Building2, BookOpen, Tag, GraduationCap, Calendar, MessageSquare, ThumbsUp } from 'lucide-react'
...
export default function EditFeedback() {
    ...
    feedbackApi.getById(id)
      .then(r => {
        ...
        setForm({
          ...
          category:         fb.category || 'Training',
          trainer_name:     fb.trainer_name || '',
          session_date:     fb.session_date || '',
          rating:           fb.rating || 0,
          comments:         fb.comments || '',
          would_recommend:  fb.would_recommend ?? true,
        })
      })
      .catch(() => { toast.error('Failed to load feedback'); navigate('/admin') })
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 text-sm font-medium mb-4 transition-colors"
      >
      ...
      <div className="card overflow-hidden">
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-5">
          <h2 className="text-white font-bold text-lg">Edit Feedback #{id}</h2>
          <p className="text-amber-100 text-sm mt-0.5">Update the feedback record details below</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <InputWrap label="Full Name *" icon={User} error={errors.participant_name}>
                <input className="input-field" value={form.participant_name} onChange={e => set('participant_name', e.target.value)} />
              </InputWrap>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputWrap label="Program / Event / Product Name *" icon={BookOpen} error={errors.program_name}>
                <input className="input-field" value={form.program_name} onChange={e => set('program_name', e.target.value)} />
              </InputWrap>
              ...
              <InputWrap label="Trainer / Instructor Name" icon={GraduationCap}>
                <input className="input-field" value={form.trainer_name} onChange={e => set('trainer_name', e.target.value)} />
              </InputWrap>
              <InputWrap label="Session Date" icon={Calendar}>
                <input type="date" className="input-field" value={form.session_date} onChange={e => set('session_date', e.target.value)} />
              </InputWrap>
            </div>
          </div>
          ...
          <div>
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-4 flex items-center gap-2">
              <MessageSquare size={12} /> Feedback Content
            </h3>
            ...
            <div className="mb-4">
              ...
              <div className="bg-slate-50 rounded-xl p-4 flex flex-col items-center gap-2 border border-slate-200">
                ...
                <p className="text-sm font-semibold text-slate-600">
                  {form.rating ? ['', 'Poor', 'Fair', 'Good', 'Very Good', 'Excellent'][form.rating] : 'Click to rate'}
                </p>
              </div>
